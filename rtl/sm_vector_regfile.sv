...
module sm_vector_regfile #(
    ...
    // -------------------------------------------------------------------------
    // Decode Interface (From sm_fetch_decode)
    // -------------------------------------------------------------------------
    ...
    output reg  [63:0] op_rs2_data, // Vector Data or Immediate (broadcasted)
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (decode_valid_q) begin
                ...
                // If it's an immediate instruction, broadcast the immediate to all vector lanes.
                if (decode_is_imm_q) begin
                    // Zero-extend immediate to 32-bit and broadcast to both lanes
                    op_rs2_data <= {16'd0, decode_imm_q, 16'd0, decode_imm_q};
                end else begin
